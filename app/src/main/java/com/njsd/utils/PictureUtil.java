...
public class PictureUtil {
     ...
     * @return
     */
    public static byte[] compressImage(byte[] imgStream, int size){
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        int options = 100;
        Bitmap image = BitmapFactory.decodeByteArray(imgStream, 0, imgStream.length);
        image.compress(Bitmap.CompressFormat.JPEG, 100, baos);//质量压缩方法，这里100表示不压缩，把压缩后的数据存放到baos中

        int baosLength = baos.toByteArray().length;
        while (baosLength / 1024 > size) {//循环判断如果压缩后图片是否大于maxMemmorrySize,大于继续压缩
